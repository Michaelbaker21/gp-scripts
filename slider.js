(function(){
  var i=setInterval(function(){
    ...
    var img=document.getElementById('ba-after');
    if(!w||!b||!img)return;
    clearInterval(i);

    function init(){
      var pw=w.offsetWidth;
      if(pw===0){setTimeout(init,100);return;}
      ...
      w.querySelectorAll('span').forEach(function(s){s.remove();});

      var line=document.createElement('div');
      line.style.cssText='position:absolute;top:0;bottom:0;width:2px;border-left:2px dashed #f4c7d5;left:0;margin-left:'+Math.round(pw/2)+'px;transform:translateX(-50%);pointer-events:none;z-index:99;';
      ...
      function s(cx){
        ...
        line.style.marginLeft=(r.width*p/100).toFixed(1)+'px';
      }
      ...
      function shimmy(){
        ...
        function tick(){
          ...
          var p=50+ease;
          var cpw=w.offsetWidth;
          b.style.setProperty('clip-path','inset(0 '+(100-p).toFixed(1)+'% 0 0)','important');
          b.style.setProperty('-webkit-clip-path','inset(0 '+(100-p).toFixed(1)+'% 0 0)','important');
          line.style.marginLeft=(cpw*p/100).toFixed(1)+'px';
          if(t<1)requestAnimationFrame(tick);
          else{
            b.style.setProperty('clip-path','inset(0 50% 0 0)','important');
            line.style.marginLeft=Math.round(cpw/2)+'px';
          }
        }
